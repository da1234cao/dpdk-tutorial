...
static void cmd_obj_del_show_parsed(void *parsed_result, struct cmdline *cl,
...
cmdline_parse_token_string_t cmd_obj_action = TOKEN_STRING_INITIALIZER(
    struct cmd_obj_del_show_result, action, "show#del");
parse_token_obj_list_t cmd_obj_obj = TOKEN_OBJ_LIST_INITIALIZER(
    struct cmd_obj_del_show_result, obj, &global_obj_list);
...
static void cmd_obj_add_parsed(void *parsed_result, struct cmdline *cl,
  ...
  if (o->ip.family == AF_INET)
    snprintf(ip_str, sizeof(ip_str), NIPQUAD_FMT, NIPQUAD(o->ip.addr.ipv4));
  else
    snprintf(ip_str, sizeof(ip_str), RTE_IPV6_ADDR_FMT,
