...
static void init_port(void) {
  ...
  for (i = 0; i < nr_queues; i++) {
    ret = rte_eth_rx_queue_setup(
        port_id, i, 512, rte_eth_dev_socket_id(port_id), &rxq_conf, mbuf_pool);
    if (ret < 0) {
...
int main(int argc, char **argv) {
  ...
  mbuf_pool = rte_pktmbuf_pool_create(
      "mbuf_pool", 4096, 128, 0, RTE_MBUF_DEFAULT_BUF_SIZE, rte_socket_id());
  ...
  flow = generate_ipv4_flow(port_id, selected_queue, SRC_IP, EMPTY_MASK,
