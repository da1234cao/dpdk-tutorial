...
// profiles[x].core[i][j],表示，第j个服务能否在第i个核上运行
...
static void apply_profile(int profile_id) {
  ...
  if (p->num_cores > rte_lcore_count() - 1) {
    printf("insufficent cores to run (%s)", p->name);
    return;
  }
  ...
  for (i = 0; i < p->num_cores; i++) {
    ...
    /** 建议在rte_service_lcore_start() 之前，调用rte_service_map_lcore_set(),
     * rte_service_map_lcore_set()建立core与service的映射关系
     * 先rte_service_lcore_start(),后建立映射,core会空转
     */
    for (s = 0; s < NUM_SERVICES; s++) {
      if (rte_service_map_lcore_set(s, core, p->cores[i].mapped_services[s]))
